# Copyright (c) 2025 Max Planck Society
# License: https://bedlam2.is.tuebingen.mpg.de/license.html
from importlib import *
import humans
import sys
# Reload modules (Unreal Engine does not reload modules automatically)
reload(humans)
from humans import HumanFactory


if __name__ == '__main__':
    print("Executing script 'import_humans_worker.py'")
    input_dir = sys.argv[1]
    pool_dir = sys.argv[2]
    animation_bool = bool(int(sys.argv[3]))
    batch_index = int(sys.argv[4])
    num_batches = int(sys.argv[5])
    human_factory = HumanFactory(pool_dir)
    # Saves every BATCH_SIZE humans to avoid memory issues
    human_factory.import_humans_to_project_worker(input_dir, animation_bool, batch_index, num_batches)
    print("Finished executing script 'import_humans_worker.py'")
